//! **W13 — 통합 증인 (Phase E / T).** `tests/`의 독립 바이너리 · **프로덕션 공개 API만** 쓴다.
//!
//! > ⚠ **Phase G(W13-G)는 여기 없다.** 무덤 복구는 **결정적 훅 park**(`pre_recover_grave`)로만 신뢰성
//! > 있게 조율되는데, 훅을 심는 `Store::with_hooks`가 `#[cfg(test)]` = **crate-private**이라 통합
//! > 바이너리에서는 닿지 못한다. 그래서 W13-G는 **lib 테스트**
//! > (`src/store/pins/tests/recover_graves_production_seam.rs::phase_g_recover_graves_survives_vanishing_graves`)로
//! > 옮겼다 — 동시성 랑데부(busy-spin)를 훅 park로 대체해 green.sha의 5/5 RED(하이젠버그)를 없앤다.
//!
